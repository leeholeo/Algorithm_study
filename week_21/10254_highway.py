...
class RotatingCalipers:
    ...
    def rotating_calipers(N, idxes, convex_hull):
        ...
        while now_flag or opposite_flag:
            now_distance = RotatingCalipers.square_distance(convex_hull[now], convex_hull[opposite])
            if now_distance > max_distance:
                max_distance = now_distance
                max_points = (convex_hull[now], convex_hull[opposite])
...
for _ in range(T):
    ...
    convex_hull = ConvexHull.endpoint_convex_hull(n, vertex)
    for x, y in RotatingCalipers.furthest_distance(convex_hull):
        print(x, y, end=' ')
    print()
